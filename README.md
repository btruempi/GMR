...
| **Money Flow** | A financial knowledge graph. Per company: a node-and-edge money-flow (revenue in → cost/R&D/SG&A/taxes/net income + capex/dividends/buybacks out) built from the latest annual **SEC filing** (SEC XBRL company-facts API). Per basket: a sector-clustered network, nodes sized by market cap and colored by money-flow (accumulating/distributing). |
