...
| **Indexes** | Build weighted baskets for any vertical (from a watchlist or preset, equal- or market-cap-weighted), track each as a composite you can activate across the Dashboard/Backtest/Heatmap. Archive or delete to declutter. |
| **Money Flow** | A financial knowledge graph in three views. **Relationships:** a traversable web of how companies connect — supply-chain (vertical), competitor (horizontal), B2B customer, licensing, partnership, and M&A edges (curated from public filings/news + industry data); click any node to re-center and follow the web. **Company spend:** a per-company money-flow (revenue → cost/R&D/SG&A/taxes/net income + capex/dividends/buybacks) from the latest annual **SEC filing** (SEC XBRL). **Aggregate:** pools SEC filings across a vertical/index/watchlist or the entire tracked market, broken down by sector, with sector→company drill-down. |
