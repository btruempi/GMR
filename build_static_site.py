...
RENDERERS.moneyflow = function(root){
  ...
  root.innerHTML =
    "<div class='card'>" +
      "<div class='flex-between'><div><h2>Money Flow &mdash; Financial Knowledge Graph</h2>" +
        "<p class='muted small'>A node-and-edge view of where a company&#39;s money comes from and where it goes, built from its latest annual <b>SEC filing</b> (10-K/20-F/40-F via the SEC XBRL API). Or switch to the basket view to see where market money is flowing across a group.</p></div></div>" +
      "<div class='row'>" +
        "<div class='seg' id='kg-mode'><button data-kg='company'" + (mode==="company"?" class='on'":"") + ">Company flow</button><button data-kg='basket'" + (mode==="basket"?" class='on'":"") + ">Basket network</button></div>" +
        (mode==="company"
  ...
  $all("#kg-mode button", root).forEach(function(b){ b.addEventListener("click", function(){ lsSet("kgMode", b.getAttribute("data-kg")); showTab("moneyflow"); }); });
  if (mode==="company"){
...
function drawCompanyFlow(root, ticker, d){
  ...
  var revY = H/2, cx = 300, coW = 150, coH = 90, coY = H/2 - coH/2, coX = cx;
  ...
  svg += "<g><rect x='" + coX + "' y='" + coY + "' width='" + coW + "' height='" + coH + "' rx='10' fill='var(--panel2)' stroke='#4fd1c5' stroke-width='1.5'/>" +
    "<text x='" + (coX+coW/2) + "' y='" + (coY+38) + "' text-anchor='middle' fill='#e7edf7' font-size='17' font-weight='800'>" + esc(ticker) + "</text>" +
    "<text x='" + (coX+coW/2) + "' y='" + (coY+60) + "' text-anchor='middle' fill='#93a2bb' font-size='11'>FY " + esc((d.fy||"").slice(0,4)) + "</text></g>";
  // out nodes
  outs.forEach(function(o, i){
    var y = pad + slotH*i + slotH/2;
    var pct = rev ? (o.val/rev*100) : 0;
    svg += "<g class='kg-node' style='cursor:default'>" +
      "<rect x='" + rightX + "' y='" + (y-18) + "' width='" + nodeW + "' height='36' rx='7' fill='" + o.color + "' fill-opacity='.16' stroke='" + o.color + "' stroke-opacity='.6'/>" +
  ...
  $("#kg-open-co").addEventListener("click", function(){ CO_TICKER = ticker; lsSet("companiesTicker", ticker); showTab("companies"); });
}
