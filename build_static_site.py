...
function renderWatchlistChart(list){
  ...
  wrap.innerHTML =
    ...
    "<div id='wl-adv-chart'></div>";
  getSeries(ticker).then(function(s){
    // reuse the full Yahoo-style component; the per-list indicator toggles seed
    // which overlays start on, but the in-chart controls take over from there.
...
function renderAdvancedChart(root, ticker, fullSeries, stateKey){
  ...
  var ctx = { root:root, ticker:ticker, series:fullSeries, state:state, stateKey:stateKey, charts:{} };
  ...
  bindSeg("#adv-range", "data-range", function(v){ state.range=v; saveState(); $all("#adv-range button",root).forEach(function(x){x.className=(x.getAttribute("data-range")===v)?"on":"";}); drawAdvanced(ctx); });
...
function sliceAdv(series, range){
  var n = series.closes.length, start = 0;
  ...
  function sl(a){ return a ? a.slice(start) : null; }
  return { dates:sl(series.dates), opens:sl(series.opens), highs:sl(series.highs), lows:sl(series.lows), closes:sl(series.closes), volumes:sl(series.volumes), synthetic:series.synthetic };
}
...
function drawAdvanced(ctx){
  var s = sliceAdv(ctx.series, ctx.state.range);
  var labels = s.dates;
  var st = ctx.state;
...
function wireCrosshair(ctx, s){
  var charts = [ctx.charts.price, ctx.charts.volume, ctx.charts.osc];
  function handler(evt){
    var chart = this;
    var rect = chart.canvas.getBoundingClientRect();
    var x = (evt.touches ? evt.touches[0].clientX : evt.clientX) - rect.left;
    var idx = Math.round(chart.scales.x.getValueForPixel(x));
    if (idx < 0) idx = 0; if (idx >= s.closes.length) idx = s.closes.length-1;
    charts.forEach(function(c){ if(c){ c.$hoverIndex = idx; c.update("none"); } });
    updateReadout(ctx, s, idx);
  }
  function clear(){
    charts.forEach(function(c){ if(c){ c.$hoverIndex = null; c.update("none"); } });
    updateReadout(ctx, s, s.closes.length-1);
  }
  charts.forEach(function(c){
    if (!c) return;
    c.canvas.onmousemove = handler.bind(c);
    c.canvas.ontouchmove = handler.bind(c);
    c.canvas.onmouseleave = clear;
  });
}
...
APP_JS = JS_OPEN + JS_CORE + JS_PROXY + JS_ROUTER + JS_DASHBOARD + JS_WATCHLISTS + JS_CHART + JS_COMPANIES + JS_OPTIMIZER + JS_BACKTEST + JS_UPDATES + JS_PREIPO + JS_PROFILE + JS_GITHUB + JS_ALERTS + JS_METHODOLOGY + JS_CLOSE
